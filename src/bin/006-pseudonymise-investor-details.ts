import {Address, CryptoUtils, InvestorDetails, InvestorDetailsRepository, LoggingUtils} from 'common';
...
export async function pseudonymisation() {
    ...
    const investorDetailsRepository = await app.getRepository(InvestorDetailsRepository);
    ...
    async function processBatch(batchObject: any){
        ...
        const keysToEncode = [
            ...
            {birthDate: InvestorDetails.definition.properties.birthDate}
        ]
        const encodedData = CryptoUtils.encodeDateObjectPseudonym(investorDetailsArray, keysToEncode);
